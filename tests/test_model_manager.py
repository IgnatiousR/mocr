...
def test_default_model_paths_resolve_under_models(monkeypatch):
    ...
    assert model_manager.default_inpaint_model_path("migan").name == "migan_traced.pt"
    assert "inpaint" in model_manager.default_inpaint_model_path("big-lama").parts
...
def test_fugumt_default_model_path_is_directory(monkeypatch):
    monkeypatch.delenv("TRANSLATION_MODEL_PATH", raising=False)

    translation = model_manager.default_translation_model_path("fugumt")

    assert translation.name == "fugumt-ja-en"
    assert "translation" in translation.parts


def test_translation_presets_describe_backend_path_repo_and_file():
    gemma = model_manager.get_translation_preset("gemma-gguf")
    sugoi = model_manager.get_translation_preset("sugoi-v4")
    fugumt = model_manager.get_translation_preset("fugumt")

    assert gemma.label == "Gemma 2 2B GGUF (llama.cpp)"
    assert gemma.backend == "llama"
    assert gemma.path.endswith(DEFAULT_TRANSLATION_FILE)
    assert gemma.repo == model_manager.DEFAULT_TRANSLATION_REPO
    assert gemma.file == DEFAULT_TRANSLATION_FILE
    assert sugoi.backend == "sugoi"
    assert sugoi.repo == model_manager.DEFAULT_SUGOI_REPO
    assert fugumt.backend == "fugumt"
    assert fugumt.repo == model_manager.DEFAULT_FUGUMT_REPO


def test_llama_backend_ignores_sugoi_env_path_and_repo(monkeypatch):
    monkeypatch.setenv("TRANSLATION_MODEL_PATH", "models/translation/sugoi-v4-ja-en-ctranslate2")
    monkeypatch.setenv("TRANSLATION_MODEL_REPO", model_manager.DEFAULT_SUGOI_REPO)

    translation = model_manager.default_translation_model_path("llama")
    repo = model_manager.get_translation_model_repo("llama")

    assert translation.name == DEFAULT_TRANSLATION_FILE
    assert repo == model_manager.DEFAULT_TRANSLATION_REPO


def test_empty_translation_model_file_uses_gemma_default(monkeypatch):
    monkeypatch.setenv("TRANSLATION_MODEL_FILE", "")

    assert model_manager.get_translation_model_file() == DEFAULT_TRANSLATION_FILE
...
def test_fugumt_backend_ignores_other_default_env_paths(monkeypatch):
    monkeypatch.setenv("TRANSLATION_MODEL_PATH", "models/translation/sugoi-v4-ja-en-ctranslate2")
    monkeypatch.setenv("TRANSLATION_MODEL_REPO", model_manager.DEFAULT_SUGOI_REPO)

    translation = model_manager.default_translation_model_path("fugumt")
    repo = model_manager.get_translation_model_repo("fugumt")

    assert translation.name == "fugumt-ja-en"
    assert repo == model_manager.DEFAULT_FUGUMT_REPO
...
def test_missing_model_messages_are_actionable():
    ...
    assert "Download inpainter model first" in inpaint_missing_message("migan", "models/inpaint/missing.pt")
...
def test_ensure_model_dirs_creates_project_folders():
    ...
    assert model_manager.INPAINT_DIR.exists()
...
def test_download_fugumt_model_can_be_mocked(tmp_path, monkeypatch):
    destination = tmp_path / "models" / "translation" / "fugumt-ja-en"
    ...
    path = download_translation_model("fugumt")

    assert path == destination
    assert (path / "config.json").read_text(encoding="utf-8") == "{}"
...
def test_download_inpaint_model_can_be_mocked(tmp_path, monkeypatch):
    destination = tmp_path / "models" / "inpaint" / "migan_traced.pt"
    ...
    monkeypatch.setattr(model_manager, "INPAINT_DIR", tmp_path / "models" / "inpaint")
    monkeypatch.setenv("INPAINTER_BACKEND", "migan")
    monkeypatch.setenv("INPAINT_MODEL_PATH", str(destination))
    monkeypatch.setattr(model_manager.urllib.request, "urlopen", lambda url: FakeResponse())

    path = download_inpaint_model("migan")
